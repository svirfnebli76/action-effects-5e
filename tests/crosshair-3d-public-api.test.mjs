...
test("Checkpoint 4 exposes canonical low-level and configured 3D placement APIs and removes legacy crosshairs", () => {
  ...
  assert.match(apiSource, /configuration:\s*Object\.freeze/);
  assert.match(apiSource, /configured:\s*Object\.freeze/);
  assert.match(apiSource, /showConfigured:\s*\(options\)\s*=>\s*crosshairs3dIntegration\.show\(options\)/);
  assert.match(apiSource, /getCatPropagationConfig/);
  ...
  assert.match(mainSource, /new Crosshair3dIntegrationService/);
