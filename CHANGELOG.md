...
- Revised the live SME integration after the first Foundry workflow gate: damage Activity replacement is now validated/applied at SME `beforeDamageRoll` (`midi-qol.preDamageRoll`) rather than `preTargeting`, because the earlier Activity can be rebuilt before damage while Midi awaits the pre-damage hook immediately before rolling. The live gate also treats `preTargetDamageApplication` as settings/outcome dependent, and `SpellModifierEventAdapter` now normalizes Midi's canonical `damageItem` argument while retaining `ditem` as a compatibility alias.
...
- Added Foundry-only SME validation: `runSpellModifierEngineFoundationTest()` covers generic registry/discovery/choice/conflict/session/rollback/event-adapter behavior, while `runSpellModifierEngineLiveActivitySubstitutionTest()` executes a disposable synthetic spell through the real Midi/CAT/D&D5e workflow and proves `beforeDamageRoll` fire→cold Activity substitution reaches the actual D&D5e damage roll, phase propagation, conditional per-target damage-application mapping, workflow-local state mirroring, and cleanup.
