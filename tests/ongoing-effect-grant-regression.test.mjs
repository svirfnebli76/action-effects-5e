    ...
    flags: {
      [MODULE_ID]: {
        [ONGOING_ACTION_ITEM_FLAG]: {
          ...
          activityIdentifier: "escape-fixture",
...
test("OngoingEffectService serializes concurrent grant requests for one exact Active Effect UUID", async () => {
  ...
  const effect = {
    ...
    id: "effect-a",
    uuid: `${actor.uuid}.ActiveEffect.effect-a`,
...
test("OngoingEffectService teardown removes every duplicate helper for one effect without touching another overlapping effect", async () => {
  const previousGame = globalThis.game;
  globalThis.game = { user: { id: "gm", isGM: true }, settings: { get: () => false } };
  const service = makeOngoingService();
  ...
  const effectB = {
    documentName: "ActiveEffect",
    id: "effect-b",
    uuid: `${actor.uuid}.ActiveEffect.effect-b`,
    parent: actor,
    flags: {
      [MODULE_ID]: {
        [ONGOING_ACTION_EFFECT_FLAG]: {
  ...
  try {
    ...
    assert.equal(actor.items.has("b1"), true, "overlapping effect B helper must survive effect A teardown");
