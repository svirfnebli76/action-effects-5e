...
## v0.4.4.13 Action Effects 3D Crosshairs — live placement and targeting
...
v0.4.4.13 retains the v0.4.4.12 Alt/focus modifier hardening while removing `pointermove` as an authoritative Ctrl/Shift source. Keyboard keydown/keyup, wheel events, and blur own modifier synchronization; this prevents Foundry/Sequencer pointer activity with unreliable modifier flags from momentarily forcing ELEVATE back to MOVE and flickering the Crosshair Elevation Gauge.
