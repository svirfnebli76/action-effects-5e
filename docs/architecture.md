...
The preferred database key is `eskie.ui.ability_check.d20.01.roll.default.white`, tinted `#1FC91C` by the effect. The service checks the Sequencer database at use time and falls back to core `icons/vtt-512.png` when that key is unavailable. Missing/broken Sequencer rendering is advisory only: it is logged and the underlying workflow continues.
